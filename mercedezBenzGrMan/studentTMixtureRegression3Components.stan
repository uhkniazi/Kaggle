...
parameters { // the parameters to track
    ...
    vector[(Ncol-1)] betasMix3; // regression parameters for each mixture component
...
transformed parameters { // calculated parameters
    ...
    vector[Ntotal] muMix3; // number of fitted values
...
model {
  ...
  betasMix3 ~ normal(0, betaSigma[3]);
  ...
  for(n in 1:Ntotal){
    ...
    ps[3] = log(iMixWeights[3]) + student_t_lpdf(y[n] | nu[3], mu[3]+muMix3[n], sigma[3]);
